...
| `smoke-test.html` | 118 tests, helper is `ok(label, cond)` — open in a browser, read the list at the bottom |
...
- **Verify by opening `smoke-test.html` in a browser** and confirming the count and that
  ...
  grown 34 → 58 → 70 → 81 → 90 → 112 → 118 across stages, so it goes in the commit.
...
- **One entry per day, one store.** `entries[dateKey]` holds `{ values, note, starred,
  closedAt, tasks }`. `entry.note` *is* the journal entry — the Day tab and Journal tab
