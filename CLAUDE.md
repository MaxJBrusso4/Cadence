...
| `smoke-test.html` | 112 tests, helper is `ok(label, cond)` — open in a browser, read the list at the bottom |
...
- **Verify by opening `smoke-test.html` in a browser** and confirming the count and that
  ...
  grown 34 → 58 → 70 → 81 → 90 → 112 across stages, so it goes in the commit.
